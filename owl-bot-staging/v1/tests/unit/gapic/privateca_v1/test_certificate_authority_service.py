...
from google.cloud.security.privateca_v1.services.certificate_authority_service.transports.base import _API_CORE_VERSION
from google.cloud.security.privateca_v1.services.certificate_authority_service.transports.base import _GOOGLE_AUTH_VERSION
...
def test_create_certificate_template_flattened():
    ...
    with mock.patch.object(
        ...
        _, args, _ = call.mock_calls[0]
        assert args[0].parent == 'parent_value'
        assert args[0].certificate_template == resources.CertificateTemplate(name='name_value')
...
def test_certificate_authority_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.security.privateca_v1.services.certificate_authority_service.transports.CertificateAuthorityServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_certificate_authority_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.security.privateca_v1.services.certificate_authority_service.transports.CertificateAuthorityServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.CertificateAuthorityServiceTransport()
        adc.assert_called_once()
...
@requires_google_auth_lt_1_25_0
def test_certificate_authority_service_transport_auth_adc_old_google_auth(transport_class):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(auth, "default", autospec=True) as adc:
        adc.return_value = (credentials.AnonymousCredentials(), None)
...
def test_certificate_authority_service_transport_create_channel_old_api_core(transport_class, grpc_helpers):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(auth, "default", autospec=True) as adc, mock.patch.object(
        grpc_helpers, "create_channel", autospec=True
...
@requires_api_core_lt_1_26_0
def test_certificate_authority_service_transport_create_channel_user_scopes(transport_class, grpc_helpers):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(auth, "default", autospec=True) as adc, mock.patch.object(
        grpc_helpers, "create_channel", autospec=True
