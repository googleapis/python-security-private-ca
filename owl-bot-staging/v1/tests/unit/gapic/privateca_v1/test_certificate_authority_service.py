...
async def test_list_ca_pools_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_ca_pools(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
async def test_list_certificate_templates_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_certificate_templates(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_delete_certificate_authority_rest_required_fields(request_type=service.DeleteCertificateAuthorityRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_certificate_authority._get_unset_required_fields(jsonified_request)
    # Check that path parameters and body parameters are not mixing in.
    assert not set(unset_fields) - set(("ignore_active_certificates", "request_id", "skip_grace_period", ))
    jsonified_request.update(unset_fields)
...
def test_delete_certificate_authority_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("ignoreActiveCertificates", "requestId", "skipGracePeriod", )) & set(("name", )))
